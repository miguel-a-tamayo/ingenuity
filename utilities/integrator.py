...
def integrateRk4(derivative: callable, state: np.array, t: float, dt: float, *args):
    """
    integrateRk4(derivative: callable, state: np.array, t: float, dt: float, *args)

    Integrates a state using Runge-Kutta "RK4" method
    ...
    k4 = derivative(t + dt, state + (dt * k3), *args)
